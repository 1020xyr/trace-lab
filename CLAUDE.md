...
- **参考模板：** `docs/blktrace/reading/` 是已完成的范例，其他工具的 reading 文件应遵循相同风格：
  - 文件头部用 `=====` 框注释标注原始路径、阅读要点、预计时间
  - 用 `★` 标记核心重点
  - 表格对照（源码 vs 含义、函数 vs 行为）
  - ASCII 流程图展示数据流
  - 底部总结调用链和完整映射表
- **输出展示文件**（如 `10_command_output_demo.md`）：每个命令实际执行、展示真实输出片段、逐行注解
...
1. **结论核查：** 给出技术结论前，必须通过源码验证，不能凭记忆或推测。尤其是涉及"默认值"、"默认行为"的结论，必须找到源码中的初始化/赋值语句确认。宁可多 grep 几行源码，也不给出未经验证的结论。（教训：fio Q3 中错误声称 `iodepth_low` 默认为 iodepth 的 1/8，实际源码 `init.c:810` 显示默认 `iodepth_low = iodepth`。）

2. **QA 自动追加：** 每当用户提出技术问题并得到解答后，**必须自动**将该问答追加到对应工具的 `QA.md` 文件中，无需询问用户是否需要。例如：
- fio 相关问题 → `docs/fio/QA.md`
- blktrace 相关问题 → `docs/blktrace/QA.md`
- 如果涉及多个工具，追加到最相关的工具 QA 中
