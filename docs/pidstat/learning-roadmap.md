...
### ★ pidstat + perf 联动 — 完整 CPU 诊断链路

```
★ pidstat 定位"谁在消耗 CPU" → perf 定位"CPU 消耗在哪个函数"
  这是从进程级到函数级的完整诊断链路。
...
联动流程：
  ...
  │   → 检测 false sharing（多核共享 cache line 争用）       │
  │                                                          │
  │ 阶段 3：综合判断                                         │
  │                                                          │
  │   pidstat %usr 高 + perf IPC 低                         │
  │     = 用户态计算密集但效率低 → 优化算法                  │
  │                                                          │
  │   pidstat %system 高 + perf 热点在 syscall              │
  │     = 系统调用开销大 → 减少 syscall 频率                 │
  │                                                          │
  │   pidstat cswch/s 高 + perf 热点在锁函数                │
  │     = 锁竞争严重 → 减少锁粒度或用无锁算法               │
  │                                                          │
  │   pidstat nvcswch/s 高 + perf CPU 利用率高              │
  │     = CPU 过载 → 扩容或降低并发                         │
  └──────────────────────────────────────────────────────────┘

★ 推荐阅读顺序：
  1. pidstat/reading/ → 掌握进程级定位
  2. perf/reading/ → 掌握函数级分析
  3. cpu_arch/reading/ → 理解微架构根因
```
