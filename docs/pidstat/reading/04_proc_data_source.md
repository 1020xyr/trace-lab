...
# 验证 rss 单位转换（页大小 4KB）
echo "rss_KB = 3158 × 4 = $((3158 * 4))"
rss_KB = 3158 × 4 = 12632             ← 与 status 的 VmRSS 一致 ✓
```
...
pidstat RSS = 12632  ←→ /proc/1/status VmRSS  = 12632 kB   ✓ 精确一致
...
# 第三步：在 dd 运行期间捕获 pidstat -d 输出
$ pidstat -d -p $(pgrep -n dd) 1 1 2>/dev/null
```
