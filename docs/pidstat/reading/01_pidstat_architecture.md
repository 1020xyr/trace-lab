...
```c
/* 源码位置：src/linux-5.10/fs/proc/array.c:374-380 */
/* ★ /proc/[pid]/status 中上下文切换部分的输出函数 */
static inline void task_context_switch_counts(struct seq_file *m,
                        struct task_struct *p)
{
    /* ★ 直接从 task_struct 读取 nvcsw / nivcsw */
...
```c
/* 源码位置：src/linux-5.10/fs/proc/array.c:405-429 */
/* ★ /proc/[pid]/status 的顶层入口函数 —— 依次调用多个子函数输出 */
            ...
            struct pid *pid, struct task_struct *task)
{
    ...
    seq_puts(m, "Name:\t");
    proc_task_name(m, task, true);       // Name: 进程名
    seq_putc(m, '\n');

    task_state(m, ns, pid, task);        // State/Pid/PPid/Uid/Gid 等

    if (mm) {
        task_mem(m, mm);                 // ★ VmPeak/VmSize/VmRSS/VmSwap 等
        task_core_dumping(m, mm);
        task_thp_status(m, mm);
        mmput(mm);
    }
    task_sig(m, task);                   // 信号相关
    task_cap(m, task);                   // capabilities
    task_seccomp(m, task);              // seccomp
    task_cpus_allowed(m, task);         // CPU 亲和性
    cpuset_task_status_allowed(m, task);
    task_context_switch_counts(m, task); // ★ 上下文切换计数（最后输出）
    return 0;
}
```

> **源码验证：** `nvcsw`（自愿切换）和 `nivcsw`（非自愿切换）直接取自
> `task_struct` 的计数器，每次上下文切换时由内核调度器递增。
> `task_mem()` 负责输出 VmSize/VmRSS 等内存字段，其底层读取 `mm_struct` 的计数器。
...
**内核源码：`proc_pid_schedstat()` 输出逻辑**

```c
/* 源码位置：src/linux-5.10/fs/proc/base.c:475-487 */
/* ★ /proc/[pid]/schedstat 文件的核心输出函数 */
                  ...
                  struct pid *pid, struct task_struct *task)
{
    if (unlikely(!sched_info_on()))
        ...
        /* ★ 三个字段，以空格分隔：
         *   字段 1: 累计执行时间（纳秒）—— task->se.sum_exec_runtime
         *   字段 2: ★ 等待被调度的时间（纳秒）—— task->sched_info.run_delay
         *   字段 3: 被调度到 CPU 上运行的次数 —— task->sched_info.pcount */
        seq_printf(m, "%llu %llu %lu\n",
           (unsigned long long)task->se.sum_exec_runtime,    // 字段 1: 运行时间
           (unsigned long long)task->sched_info.run_delay,   // 字段 2: ★ 等待时间
           task->sched_info.pcount);                         // 字段 3: 调度次数
    return 0;
}
#endif
```

> **源码验证：** pidstat 的 `%wait` 来自 `task->sched_info.run_delay`（字段 2），
> 这是进程在就绪队列中等待 CPU 的累积纳秒数。
> `task->se.sum_exec_runtime`（字段 1）是实际运行时间，可用于交叉验证 `%CPU`。
