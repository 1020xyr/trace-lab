...
★ **重要：队列打满时增大 iodepth 通常无用！**

队列已满 → 设备以最大速度处理 → 增大 iodepth 只是让队列更长：
- 设备速度不变 → IOPS 不变
- 队列更长 → 延迟翻倍（0.1ms→0.2ms）
- → ★ 通常无用，只增加延迟

**唯一例外：设备内部有未被利用的并行度**（如 NVMe 多通道），增大 iodepth 可能让更多通道同时工作。但必须实测 IOPS 变化确认。

**优化方向（纠正）：**
1. ★ **实测对比**：iodepth=32/64/128 的 IOPS
   - IOPS 不变 → 设备真饱和 → 换设备/减少 I/O 量
   - IOPS 提升 → 设备有并行度 → 增大 iodepth 找拐点
2. 换更快设备
3. 改 I/O 模式（合并、大块）
